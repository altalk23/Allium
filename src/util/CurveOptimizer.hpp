...
namespace allium {
    ...
    private:
        ...
        bool hasAngle(Point p1, Point p2, Point p3) {
            return this->distance(p1, p2) > 1e-6 && this->distance(p2, p3) > 1e-6;
        }
        ...
        bool checkColinear(Point p1, Point p2, Point p3) {
            return std::abs(a(p1, p2) * b(p2, p3) - a(p2, p3) * b(p1, p2)) < 1e-6;
        }

    public:

        CurveOptimizer(float lineWidth, std::vector<Point>&& points) : m_lineWidth(lineWidth), m_points(std::move(points)) {}

        void handleExtension(std::vector<CurveOptimizer::Rect>& rects, std::vector<CurveOptimizer::Circle>& circles) {
            ...
            for (size_t i = 0; i < size - 1; ++i) {
                ...
                if (i > 0) { // we're not at the beginning, so we can try to extend the bottom side
                    ...
                    if (this->hasAngle(a1, a2, a3) && std::abs(this->angle(a1, a2, a3)) >= M_PI * 0.75) { 
                ...
                if (i < size - 2) { // we're not at the end, so we can try to extend the top side
                    ...
                    // if the angle is big enough, we can extend the corner
                    if (this->hasAngle(a2, a3, a4) && std::abs(this->angle(a2, a3, a4)) >= M_PI * 0.75) {
