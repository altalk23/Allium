	...
	struct NameTable {
		...
		uint32_t parse(const char* data, uint32_t offset, std::unordered_map<uint64_t, std::vector<std::string>>& names, uint16_t max_number_of_names = 25) {
			...
			for (auto i = 0; i < count; i++) {
				...
				if (current_name_record.size() <= nameRecord[i].nameID)
					current_name_record.resize(nameRecord[i].nameID + 1);
...
int8_t TTFFontParser::parse_data(const char* data, TTFFontParser::FontData* font_data) {
	...
	std::multimap<uint16_t, uint32_t> glyph_reverse_map;
	...
	if (kern_offset) {
		...
		for (uint16_t kern_subtable_index = 0; kern_subtable_index < num_kern_subtables; kern_subtable_index++) {
			...
			for (uint16_t kern_index = 0; kern_index < num_kern_pairs; kern_index++) {
				...
				const uint32_t kern_left_character_index = glyph_reverse_map.find(kern_left)->second;
				const uint32_t kern_right_character_index = glyph_reverse_map.find(kern_right)->second;
